...
Constrain your edits exclusively to the local workspace to draft and test your code refactoring. Ensure you explicitly decouple sandbox operations from live deployment mechanics, as physical write-authorization to the SAP backend is absent.

- Given an ATC `job_id`, create a local physical mapping directory **exactly** at: `./src/atc_runs/<job_id>/`
- **CRITICAL DISTINCTION**: Treat this specific folder as a formal deployment staging area that must be permanently preserved within `./src/` to guarantee an entirely decoupled sandbox environment explicitly separated from live deployment mechanics. Exempt this directory from all general "Workspace Sanitization" routines.
...
1. **Quick Fix First**: Always poll `sap_atc_quick_fix` (for ATC findings) or `sap_syntax_quick_fix` (for syntax errors) against the underlying `finding_uri`. These tools return read-only evaluations. If SAP provides an explicit system rewrite suggestion, aggressively use it to guide your local refactoring.
...
Because write operations onto SAP are entirely decoupled, your official task ends with preparing the deployment package for the human owner.
...
- Instruct the Human user formally indicating your local scripts within the `./src/atc_runs/<job_id>/` folder are structurally compliant, signed-off offline, and ready for a physical manual push out to their proprietary SAP deployment pipelines.
