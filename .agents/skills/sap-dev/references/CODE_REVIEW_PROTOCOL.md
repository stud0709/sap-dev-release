...
*   **Draft Comparison**: When evaluating developer modifications, compare the local draft (stored in the gitignored `./src/` sandbox) against the active backend code using the native `sap_diff_versions` tool (see [VERSIONING_AND_DIFF_GUIDE.md](./VERSIONING_AND_DIFF_GUIDE.md)). Terminal commands like `git diff` or `git log` should be avoided for the `./src/` directory because they will not return diffs or history for gitignored paths.
...
### D. Static Quality & ATC Compliance
*   **ATC Quality Findings**: Verify that the code passes ABAP Test Cockpit (ATC) static quality checks without OPEN high-priority findings.
*   **ATC Workflow Integration**: Use `sap_fetch_atc_queue` to audit open findings and inspect official remediation documentation via `sap_atc_documentation`. For automated fixes, refer to [ATC_REMEDIATION.md](./ATC_REMEDIATION.md).
...
## 🔗 5. Sub-Agent Resource Guides & Domain Manuals

*   **[ABAP Developer Guide](./DEVELOPER_GUIDE.md)**: Developer editing, push, syntax validation, dynamic RPC execution, unit testing, and debugging workflows.
*   **[Repository Lifecycle Guide](./REPOSITORY_LIFECYCLE_GUIDE.md)**: Dynamic template creation, aspect pushing, class pool includes, message classes, and mass activation.
*   **[ATC Remediation Guide](./ATC_REMEDIATION.md)**: Static quality checks, quick-fixes, and finding lifecycle management.
*   **[Versioning & Diffing Guide](./VERSIONING_AND_DIFF_GUIDE.md)**: 3-way structural diffing, local draft timelines, and remote server revision history.
*   **[MCP Tool Master Catalog](./MCP_TOOL_GUIDE.md)**: Master catalog and 74-tool navigation index.
